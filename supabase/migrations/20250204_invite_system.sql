...
-- Adds invite codes, waitlist tracking, and account status gating.
...
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS invites_remaining INT DEFAULT 2;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS invites_used INT DEFAULT 0;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS account_status TEXT DEFAULT 'waitlist';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS waitlist_joined_at TIMESTAMPTZ;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS activated_at TIMESTAMPTZ;

-- All existing users are active (grandfathered in)
UPDATE profiles SET account_status = 'active', activated_at = now()
WHERE account_status IS NULL OR account_status = 'waitlist';

-- Generate invite codes for existing users who don't have one
-- Format: first 4 chars of username (uppercased) + '-' + 4 random alphanumeric
UPDATE profiles
SET invite_code = UPPER(LEFT(COALESCE(username, 'USER'), 4)) || '-' ||
  SUBSTR(MD5(RANDOM()::TEXT), 1, 4)
WHERE invite_code IS NULL AND username IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_waitlist_position ON waitlist(display_position);
...
-- Invite codes: users can only see their own
CREATE POLICY "Users can view own invite codes"
  ON invite_codes FOR SELECT
  USING (auth.uid() = owner_id);

-- Waitlist: users can view their own entry
CREATE POLICY "Users can view own waitlist entry"
  ON waitlist FOR SELECT
  USING (auth.uid() = user_id);

---------------------------------------------------
-- 5. RPC: Add to waitlist (with inflated position)
---------------------------------------------------
...
---------------------------------------------------
-- 6. RPC: Generate invite codes for a user
---------------------------------------------------
...
BEGIN
  ...
  FOR v_i IN 1..p_count LOOP
    -- Generate unique suffix (4 chars, no ambiguous chars)
    LOOP
  ...
  -- Return all active codes for this user
  RETURN QUERY
...
---------------------------------------------------
-- 10. RPC: Validate invite code (public, for login page)
---------------------------------------------------
