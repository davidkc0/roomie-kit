...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        // OneSignal Cordova plugin should automatically capture this via swizzling
        let tokenParts = deviceToken.map { data in String(format: "%02.2hhx", data) }
        let token = tokenParts.joined()
        print("[AppDelegate] Registered for push with token: \(token)")
    }
    ...
    // MARK: - Screen Orientation Support
    // Required for @capacitor/screen-orientation plugin to work with presented view controllers
