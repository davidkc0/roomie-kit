...
type SnakeLeaderboardProps = {
    isOpen: boolean;
    onClose: () => void;
};

export function SnakeLeaderboard({ isOpen, onClose }: SnakeLeaderboardProps) {
    ...
    useEffect(() => {
        ...
        if (activeTab === 'weekly') {
            ...
            supabase.rpc('get_weekly_leaderboard', { p_game: 'snake' }).then((scoresResult) => {
                if (scoresResult.error) {
                    console.error('[SnakeLeaderboard] Error fetching weekly scores:', scoresResult.error);
            ...
            supabase
                ...
                .then(({ data, error }) => {
                    if (error) {
                        console.error('[SnakeLeaderboard] Error fetching all-time scores:', error);
            ...
            supabase.rpc('get_friends_leaderboard', { p_game: 'snake', p_period: 'weekly' })
                .then(({ data, error }) => {
                    if (error) {
                        console.error('[SnakeLeaderboard] Error fetching friends scores:', error);
                        setFriendsLeaderboard([]);
                    } else {
            ...
            <div className="bg-bg-surface border border-border rounded-2xl w-full max-w-md max-h-[80vh] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in duration-300">
                ...
                <div className="flex items-center justify-between p-4 border-b border-border">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-xl font-bold text-white">Snake Leaderboard</h2>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    {loading ? (
                        ...
                        leaderboard.map(entry => {
                            ...
                            return (
                                <div
                                    key={entry.id}
                                    className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${getRankBg(entry.rank)} ${isCurrentUser ? 'ring-2 ring-green-500/50' : ''}`}
                                >
                                    {/* Rank */}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            <span className={`font-bold truncate ${isCurrentUser ? 'text-green-400' : 'text-white'}`}>
                                                {entry.username}
                                            </span>
                                            {isCurrentUser && (
